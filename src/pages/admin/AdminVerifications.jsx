...
export default function AdminVerifications() {
    ...
    const fetchPendingMentors = async () => {
        try {
            // Fetch profiles where verificationStatus is PENDING inside metadata
            const { data, error } = await supabase
                .from('Profile')
                .select('*')
                .eq('isStudyingAbroad', true) // Only mentors

            if (error) throw error

            // Client-side filter for metadata since JSONB query syntax can be tricky without specific indexes
            const pending = data.filter(p => p.metadata?.verificationStatus === 'PENDING')
    ...
    return (
        <AdminLayout>
            <div className="mb-8">
                <h1 className="text-3xl font-bold text-white">Pending Verifications</h1>
            ...
            {loading ? (
                ...
                <div className="glass-card p-12 rounded-2xl flex flex-col items-center justify-center text-center opacity-70">
                    ...
                    <p className="text-gray-400 mt-2">All caught up! No mentors are currently waiting for verification.</p>
                </div>
            ) : (
                <div className="grid gap-6">
                    {pendingMentors.map(mentor => (
                        <div key={mentor.id} className="glass-card p-6 rounded-2xl border border-white/10 flex flex-col lg:flex-row gap-6">
